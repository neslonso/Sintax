...
/******************************************************************************/
	...
	public function esSpainPeninsular() {
		if (!$this->paisToIsoCode()=='ES') {
			...
			if (
				substr($this->cp, 0,2)=="07" || //Illes Baleares
				substr($this->cp, 0,2)=="35" || //Las Palmas
				substr($this->cp, 0,2)=="38" || //S.C. Tenerife
				substr($this->cp, 0,2)=="51" || //Ceuta
				substr($this->cp, 0,2)=="52" //Melilla
			) {
				$result=false;
			} else {
				$result=true;
	...
	public function esPortugalPenisular() {
		...
		if ($this->paisToIsoCode()=='PT') {
			if (substr($this->cp, 0,1)=="9") { //Azores y Madeira
				$result=false;
			} else {
				$result=true;
			}
